...
async def get_forex_klines(symbol, interval="5min", limit=100):
    if not TWELVE_KEY:
        logger.error("TWELVE_KEY not set!")
        return None
    try:
        # TwelveData формат: EUR/USD
        pair = symbol[:3] + "/" + symbol[3:]
        async with aiohttp.ClientSession() as s:
            async with s.get(
                f"https://api.twelvedata.com/time_series",
                params={"symbol": pair, "interval": interval, "outputsize": limit, "apikey": TWELVE_KEY},
...
async def analyze_forex(symbol, timeframe="5min"):
    candles = await get_forex_klines(symbol, timeframe, 100)
    if not candles or len(candles) < 30:
        return None

    # TwelveData повертає від новіших до старіших — перевертаємо
    ...
    # Обсяг недоступний для форекс — передаємо 1.0
    ...
    tf_label = {"5min":"5м","15min":"15м","1h":"1г","4h":"4г"}.get(timeframe, timeframe)
    pair_name = FOREX_PAIRS.get(symbol, symbol)
    is_jpy = "JPY" in symbol
    return {"symbol": pair_name, "raw_symbol": symbol, "market": "forex", "price": c[-1],
            "score": sc, "direction": dir, "reasons": rs, "entry": entry,
            "sl": sl, "tp1": tp1, "tp2": tp2, "tp3": tp3, "atr": atr,
            "timeframe": tf_label, "time": now_kyiv().strftime("%H:%M:%S"), "is_jpy": is_jpy}
