...
FOREX_PAIRS = {"EURUSD":"EUR/USD","GBPUSD":"GBP/USD","USDJPY":"USD/JPY","AUDUSD":"AUD/USD","USDCAD":"USD/CAD","EURGBP":"EUR/GBP"}
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if section == "signals":
        ...
        if text == "📊 Топ можливості":
            msg = await update.message.reply_text("🔍 Скануємо...")
            res = []
            for p in CRYPTO_PAIRS[:6]:
                ...
                await asyncio.sleep(0.3)
            res.sort(key=lambda x:-x["score"])
            if not res: await msg.edit_text("😐 Немає сигналів >65%"); return
            t = "🏆 *Топ можливості:*\n\n"
            for s in res[:5]:
                e="🟢" if s["direction"]=="LONG" else "🔴"
                t += f"{e} *{s['symbol']}* — {s['score']}% | ${s['price']:.4f}\n"
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("sig_fx_"):
        ...
        await q.edit_message_text(f"💱 Аналізую {name}...\n\n_(Форекс аналіз в розробці — незабаром!)_", parse_mode="Markdown"); return
