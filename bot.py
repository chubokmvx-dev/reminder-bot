...
def signals_kb():
    return ReplyKeyboardMarkup([
        ["💱 Форекс сигнали", "📊 Топ можливості"],
        ["📡 Авто-сигнали", "🏠 Головне меню"],
    ], resize_keyboard=True)
...
# Rate limiter — макс 7 запитів/хв (безпечно в межах ліміту 8)
_td_request_times = []

async def _rate_limit():
    global _td_request_times
    now = asyncio.get_event_loop().time()
    _td_request_times = [t for t in _td_request_times if now - t < 62]
    if len(_td_request_times) >= 7:
        wait = 62 - (now - _td_request_times[0]) + 0.5
        if wait > 0:
            logger.info(f"TwelveData rate limit: чекаємо {wait:.1f}с")
            await asyncio.sleep(wait)
        _td_request_times = []
    _td_request_times.append(asyncio.get_event_loop().time())
...
async def analyze_forex_mtf(symbol):
    ...
    # Мінімум 2 з 3 таймфреймів мають збігатись
    if long_count >= 2:
        final_dir = "LONG"
    ...
    elif long_count == 1 and directions:
        # Fallback — хоча б 1 таймфрейм дає сигнал
        final_dir = "LONG"
        alignment = 1
    elif short_count == 1 and directions:
        final_dir = "SHORT"
        alignment = 1
    else:
        return None

    # Скор залежно від узгодженості
    ...
    if alignment == 3:
        ...
        all_reasons.insert(0, f"✅ 2/3 таймфреймів збігаються")
    else:
        final_score = max(0, base_score - 10)
        all_reasons.insert(0, f"⚡ Сигнал на 1 таймфреймі")
    ...
    if "1h" in results:
        ...
        if h1["dir"] != final_dir:
            ...
            all_reasons.append("⚠️ Проти тренду 1г")
        else:
            all_reasons.append("📈 За трендом 1г ✅")
    ...
    if section == "signals":
        if text == "💱 Форекс сигнали":
            ...
            for i, sym in enumerate(FOREX_SCAN_PAIRS):
                ...
                if s and s["score"] >= 55: res.append(s)
        ...
        if text == "📊 Топ можливості":
            msg = await update.message.reply_text("🔍 Шукаю найкращі сигнали...")
            res = []
            for i, sym in enumerate(FOREX_SCAN_PAIRS):
                await msg.edit_text(f"🔍 Аналізую {i+1}/{len(FOREX_SCAN_PAIRS)}: {sym}")
                s = await analyze_forex(sym)
                if s and s["score"] >= 65: res.append(s)
                await asyncio.sleep(0.3)
            res.sort(key=lambda x: -x["score"])
            if not res:
                await msg.edit_text("😐 Немає сигналів >65%\n\nРинок нейтральний — краще не торгувати!")
                return
            t = "🏆 *Топ Pocket Option сигнали:*\n\n"
            for s in res[:5]:
                e = "🟢⬆️" if s["direction"] == "LONG" else "🔴⬇️"
                stars = "⭐" * s.get("alignment", 1)
                t += f"{e} *{s['symbol']}* — {s['score']}% {stars}\n"
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton(f"📊 {r['symbol']}", callback_data=f"sig_fx_{r['raw_symbol']}_5min")]
                for r in res[:5]
        ...
        if text == "📡 Авто-сигнали":
            u["signals"]["active"] = not u["signals"].get("active", False)
            save_data(data)
            status = "✅ Увімкнено" if u["signals"]["active"] else "❌ Вимкнено"
            info = (
                f"📡 Авто-сигнали: {status}\n\n"
            )
            if u["signals"]["active"]:
                info += (
                    "Сканую всі 28 пар кожні 5 хвилин\n"
                    "Надішлю сигнал коли скор ≥70%\n\n"
                    "⭐⭐⭐ — найкращі сигнали\n"
                    "⭐⭐ — хороші сигнали\n"
                    "⭐ — слабші сигнали"
                )
            else:
                info += "Сповіщення вимкнено"
