...
def signals_kb():
    return ReplyKeyboardMarkup([
        ["💱 Форекс сигнали", "🔍 Аналіз пари"],
        ["📊 Топ можливості", "📡 Авто-сигнали"],
        ["📚 Навчання", "🧮 Калькулятор позиції"],
        ["🏠 Головне меню"],
    ], resize_keyboard=True)
...
FOREX_PAIRS = {
    ...
    "USD/CHF":"USD/CHF","AUD/USD":"AUD/USD","USD/CAD":"USD/CAD","NZD/USD":"NZD/USD",
    # Кроси EUR
    "EUR/GBP":"EUR/GBP","EUR/JPY":"EUR/JPY","EUR/CHF":"EUR/CHF",
    "EUR/AUD":"EUR/AUD","EUR/CAD":"EUR/CAD","EUR/NZD":"EUR/NZD",
    # Кроси GBP
    "GBP/JPY":"GBP/JPY","GBP/CHF":"GBP/CHF","GBP/AUD":"GBP/AUD",
    "GBP/CAD":"GBP/CAD","GBP/NZD":"GBP/NZD",
    # Кроси JPY
    "AUD/JPY":"AUD/JPY","CAD/JPY":"CAD/JPY","CHF/JPY":"CHF/JPY","NZD/JPY":"NZD/JPY",
    # Інші
    "AUD/CAD":"AUD/CAD","AUD/CHF":"AUD/CHF","AUD/NZD":"AUD/NZD",
    "CAD/CHF":"CAD/CHF","NZD/CAD":"NZD/CAD","NZD/CHF":"NZD/CHF",
}

FOREX_SCAN_PAIRS = list(FOREX_PAIRS.keys())  # всі 28 пар

# Таймфрейми для Pocket Option (експірація)
PO_EXPIRY = {
    "5min": {"label": "5м", "expiry": "5 хвилин"},
    "15min": {"label": "15м", "expiry": "15 хвилин"},
    "1h": {"label": "1г", "expiry": "1 година"},
}
...
def calc_sr_levels(candles, lookback=50):
...
async def analyze_forex_mtf(symbol):
    timeframes = ["5min", "15min", "1h"]
    tf_labels = {"5min":"5м","15min":"15м","1h":"1г"}
    results = {}
    candles_cache = {}

    # Завантажуємо всі таймфрейми
    for tf in timeframes:
        candles = await get_twelvedata_klines(symbol, tf, 100)
        if candles and len(candles) >= 30:
            candles_cache[tf] = candles
        await asyncio.sleep(0.3)

    if "5min" not in candles_cache:
        return None
    ...
    for tf in timeframes:
        if tf not in candles_cache:
            continue
        candles = candles_cache[tf]
        try:
            o = [float(c["open"]) for c in candles]
            h = [float(c["high"]) for c in candles]
            l = [float(c["low"]) for c in candles]
            ...
            sc, dir, rs = score_signal(rsi, ml2, ms, bbu, bbl, bbm, c[-1], e9, e21, e50, sk, 1.0, pats)
            results[tf] = {"dir": dir, "score": sc, "close": c[-1], "atr": atr, "reasons": rs}
            ...
            if dir != "NEUTRAL":
                ...
                label = tf_labels[tf]
                if rs:
                    all_reasons.append(f"[{label}] {rs[0]}")
        except Exception as e:
            logger.error(f"MTF error {symbol} {tf}: {e}")

    if not directions:
        return None

    # Перевіряємо узгодженість таймфреймів
    long_count = directions.count("LONG")
    short_count = directions.count("SHORT")
    total = len(directions)

    # Для якісного сигналу потрібно щоб мінімум 2 з 3 таймфреймів збігались
    if long_count >= 2:
        final_dir = "LONG"
        alignment = long_count
    elif short_count >= 2:
        final_dir = "SHORT"
        alignment = short_count
    else:
        return None  # Таймфрейми суперечать — немає сигналу

    # Бонус за повне узгодження всіх 3 таймфреймів
    base_score = sum(scores) / len(scores)
    if alignment == 3:
        final_score = min(100, base_score + 15)
        all_reasons.insert(0, "🎯 Всі 3 таймфрейми збігаються!")
    else:
        final_score = min(100, base_score + 5)
        all_reasons.insert(0, f"✅ {alignment}/3 таймфреймів збігаються")

    # Фільтр тренду — перевіряємо 1г
    if "1h" in results:
        h1 = results["1h"]
        if h1["dir"] != final_dir:
            final_score = max(0, final_score - 20)
            all_reasons.append("⚠️ Проти тренду 1г — знижений скор")
        else:
            all_reasons.append("📈 За трендом 1г — підтверджено")

    if final_score < 65:
        return None

    # Рівні входу від 5м свічок
    c5 = results.get("5min", {})
    entry = c5.get("close", 0)
    atr = c5.get("atr", entry * 0.001)

    # Рівні S/R
    candles_5m = candles_cache.get("5min", [])
    resistance, support = calc_sr_levels(candles_5m)
    ...
    if final_dir == "LONG":
        ...
        tp2 = entry + atr * 3
        tp3 = entry + atr * 5
        # Коригуємо TP до найближчого опору
        if resistance and resistance[-1] > entry:
            tp1 = min(tp1, resistance[-1] * 0.9995)
    else:
        sl = entry + atr * 1.8
        tp1 = entry - atr * 1.5
        tp2 = entry - atr * 3
        tp3 = entry - atr * 5
        if support and support[0] < entry:
            tp1 = max(tp1, support[0] * 1.0005)

    pair_name = FOREX_PAIRS.get(symbol, symbol)
    is_jpy = "JPY" in symbol

    return {
        "symbol": pair_name, "raw_symbol": symbol, "market": "forex",
        "price": entry, "score": int(final_score), "direction": final_dir,
        "reasons": all_reasons[:6], "entry": entry, "sl": sl,
        "tp1": tp1, "tp2": tp2, "tp3": tp3, "atr": atr,
        "timeframe": "5м+15м+1г", "time": now_kyiv().strftime("%H:%M:%S"),
        "is_jpy": is_jpy, "alignment": alignment
    }
...
async def analyze_forex(symbol, timeframe="5min"):
    # Якщо вибраний конкретний таймфрейм — один аналіз
    if timeframe != "5min":
        return await analyze_symbol(symbol, timeframe, "forex")
    # Для 5м — мультитаймфреймний аналіз
    return await analyze_forex_mtf(symbol)
...
def fmt_signal(sig):
    ...
    # Pocket Option формат — ВГОРУ/ВНИЗ
    if direction == "LONG":
        dir_emoji = "🟢"
        dir_text = "ВГОРУ (CALL) ⬆️"
    else:
        dir_emoji = "🔴"
        dir_text = "ВНИЗ (PUT) ⬇️"
    ...
    align = sig.get("alignment", 2)
    stars = "⭐" * align

    # Визначаємо експірацію по таймфрейму
    tf = sig.get("timeframe", "5м+15м+1г")
    if "1г" in tf or "4г" in tf:
        expiry = "⏱ Експірація: *1 година*"
    elif "15м" in tf:
        expiry = "⏱ Експірація: *15 хвилин*"
    else:
        expiry = "⏱ Експірація: *5 хвилин*"

    # Час входу
    entry_time = sig["time"]
    now = now_kyiv()
    # Наступна хвилина для входу
    entry_min = now.strftime("%H:%M")

    t = (
        f"{'='*28}\n"
        f"🎯 *POCKET OPTION СИГНАЛ* {stars}\n"
        f"{'='*28}\n\n"
        f"💱 *{sig['symbol']}*\n"
        f"{dir_emoji} *{dir_text}*\n\n"
        f"💰 Поточна ціна: `{f(sig['price'])}`\n"
        f"🕐 Час входу: *{entry_min}*\n"
        f"{expiry}\n\n"
        f"📊 *Сила сигналу:*\n"
        f"`{bar}` {score}%\n\n"
        f"📋 *Підтвердження:*\n"
    ...
    t += (
        f"\n💡 *Як торгувати:*\n"
        f"1. Відкрийте Pocket Option\n"
        f"2. Виберіть *{sig['symbol']}*\n"
        f"3. Встановіть час: {expiry.split(': ')[1]}\n"
        f"4. Натисніть *{'ВГОРУ' if direction=='LONG' else 'ВНИЗ'}*\n\n"
        f"⚠️ _Ризик не більше 2-3% від депозиту!_"
    )
    return t
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ══ СИГНАЛИ ══
    if section == "signals":
        if text == "💱 Форекс сигнали":
            msg = await update.message.reply_text("💱 Сканую 28 пар (MTF аналіз)...")
            res = []
            for i, sym in enumerate(FOREX_SCAN_PAIRS):
                await msg.edit_text(f"💱 Сканую {i+1}/{len(FOREX_SCAN_PAIRS)}: {sym}")
                s = await analyze_forex(sym)
                if s and s["score"] >= 70: res.append(s)
                await asyncio.sleep(0.5)
            if not res:
                await msg.edit_text("😐 Зараз немає якісних сигналів\n\nНемає підтвердження на 2+ таймфреймах\nЧекайте — перевіряю кожні 5 хвилин!")
                return
            res.sort(key=lambda x: -x["score"])
            t = "💱 *Pocket Option сигнали:*\n\n"
            for s in res[:6]:
                e = "🟢⬆️" if s["direction"] == "LONG" else "🔴⬇️"
                align = s.get("alignment", 2)
                stars = "⭐" * align
                t += f"{e} *{s['symbol']}* — {s['score']}% {stars}\n"
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton(f"📊 {r['symbol']}", callback_data=f"sig_fx_{r['raw_symbol']}_5min")]
                for r in res[:6]
            ])
            await msg.edit_text(t, parse_mode="Markdown", reply_markup=kb)
            return
        ...
        if text == "🧮 Калькулятор позиції":
            await update.message.reply_text(
                "🧮 *Калькулятор розміру позиції*\n\n"
                "Введіть дані у форматі:\n"
                "`депозит ризик% ціна_входу ціна_SL`\n\n"
                "Приклад:\n"
                "`1000 2 1.34500 1.34200`\n\n"
                "📌 Де:\n"
                "• *1000* — ваш депозит ($)\n"
                "• *2* — ризик (% від депозиту)\n"
                "• *1.34500* — ціна входу\n"
                "• *1.34200* — ваш стоп-лос\n\n"
                "⚠️ Рекомендований ризик: 1-2% на угоду",
                parse_mode="Markdown"
            )
            context.user_data["state"] = "calc_position"
            return

        if state == "calc_position":
            context.user_data["state"] = None
            try:
                parts = text.strip().split()
                if len(parts) != 4:
                    await update.message.reply_text("❌ Введіть 4 числа через пробіл\nПриклад: `1000 2 1.34500 1.34200`", parse_mode="Markdown")
                    return
                ...
                if risk_pct <= 0 or risk_pct > 10:
                    await update.message.reply_text("❌ Ризик має бути від 0.1 до 10%")
                    return
                if entry <= 0 or sl <= 0:
                    await update.message.reply_text("❌ Ціни мають бути більше 0")
                    return
                if entry == sl:
                    await update.message.reply_text("❌ Ціна входу і SL не можуть бути однаковими")
                    return
                ...
                # Розмір позиції в одиницях активу
                ...
                is_forex = entry < 10
                if is_forex:
                    pip_size = 0.0001 if entry < 100 else 0.01
                    sl_pips = sl_distance / pip_size
                    lot_size = risk_amount / (sl_pips * 10)  # $10 на pip для стандартного лоту
                    micro_lots = lot_size * 100

                # Різні плечі
                levers = [1, 10, 50, 100, 200, 500]

                text_result = (
                    f"🧮 *Результат розрахунку*\n\n"
                    f"💰 Депозит: ${deposit:,.0f}\n"
                    f"⚠️ Ризик: {risk_pct}% = *${risk_amount:,.2f}*\n"
                    f"📍 Вхід: {entry}\n"
                    f"🛑 SL: {sl} ({sl_pct:.3f}%)\n"
                ...
                if is_forex:
                    text_result += (
                        f"💱 *Форекс розрахунок:*\n"
                        f"SL в пунктах: {sl_pips:.1f}\n"
                        f"Розмір лоту: {lot_size:.4f}\n"
                        f"Мікро-лоти: {micro_lots:.2f}\n\n"
                    )

                text_result += f"📐 *Розмір позиції по плечу:*\n\n"
                for lev in levers:
                    max_pos = deposit * lev
                    recommended = min(position_value, max_pos)
                    if position_value <= max_pos:
                        emoji = "✅"
                    else:
                        emoji = "⚠️"
                    text_result += f"{emoji} ×{lev}: ${recommended:,.0f} ({recommended/deposit*100:.0f}% депозиту)\n"

                text_result += (
                    f"\n━━━━━━━━━━━━━━━━━━━━\n\n"
                    f"💡 *Рекомендація:*\n"
                    f"При ×{10 if is_forex else 50}: використайте ${min(position_value, deposit*10 if is_forex else deposit*50):,.0f}\n"
                    f"Макс збиток: ${risk_amount:,.2f} ({risk_pct}%)\n\n"
                    f"⚠️ _Ніколи не ризикуйте більше 2% за угоду!_"
                )

                await update.message.reply_text(text_result, parse_mode="Markdown")
            except ValueError:
                await update.message.reply_text(
                    "❌ Невірний формат. Введіть 4 числа:\n"
                    "`депозит ризик% ціна_входу ціна_SL`\n\n"
                    "Приклад: `1000 2 1.34500 1.34200`",
                    parse_mode="Markdown"
                )
            return

        # Невідомий текст в розділі сигналів — ігноруємо
        return
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("sig_detail_"):
        # Старий callback — перенаправляємо на форекс
        await q.edit_message_text("💱 Оберіть пару:", reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(name, callback_data=f"sig_fx_{sym}_5min")]
            for sym, name in list(FOREX_PAIRS.items())[:8]
        ]))
        return
...
async def signal_scheduler(app):
    sent = {}
    scan_interval = 300  # сканування кожні 5 хвилин
    while True:
        try:
            data = load_data()
            active = {uid: u for uid, u in data.items() if u.get("signals", {}).get("active")}
            if active:
                logger.info(f"Auto-signal scan: {len(active)} users active")
                for sym in FOREX_SCAN_PAIRS:
                    try:
                        s = await analyze_forex(sym)
                        if s and s["score"] >= 85:
                            # Унікальний ключ — щоб не спамити той самий сигнал
                            price_rounded = round(s["price"], 4)
                            k = f"{sym}_{s['direction']}_{price_rounded}"
                            if k not in sent:
                                ...
                                for uid in active:
                                    try:
                                        ...
                                        logger.error(f"Signal send to {uid}: {e}")
                                logger.info(f"Sent signal: {sym} {s['direction']} {s['score']}%")
                    except Exception as e:
                        logger.error(f"Signal scan error {sym}: {e}")
                    await asyncio.sleep(0.5)

            # Чистимо старі сигнали (старші 30 хвилин)
