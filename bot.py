...
async def get_klines(symbol, interval="5m", limit=100):
    # Bybit інтервали: 1,3,5,15,30,60,120,240,360,720,D,W,M
            ...
            async with s.get(
                ...
                if r.status == 200:
                    ...
                    if data.get("retCode") == 0:
                        ...
                        # Bybit повертає від нових до старих — перевертаємо
                        # Формат: [timestamp, open, high, low, close, volume, turnover]
                        raw = list(reversed(raw))
                        # Конвертуємо у формат Binance: [ts, open, high, low, close, volume]
                        return [[c[0], c[1], c[2], c[3], c[4], c[5]] for c in raw]
    except Exception as e:
        logger.error(f"Bybit klines error {symbol}: {e}")
    return None
